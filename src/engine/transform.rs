...
use std::fs;
use std::path::Path;
use super::{EngineError, plan::{Action, TransformationPlan, TemplateContext}};
                ...
                if full_path.exists() {
                    if full_path.is_dir() {
                        fs::remove_dir_all(&full_path)
                            .map_err(|e| EngineError::FileSystem(format!(
...
mod tests {
    ...
    fn test_transformation_execution() {
        ...
        fs::write(dir.path().join("README.md"), "Project: General Web App\nAuthor: {{author_name}}").unwrap();
        ...
        plan.actions.push(Action::Delete(std::path::PathBuf::from(".github")));
